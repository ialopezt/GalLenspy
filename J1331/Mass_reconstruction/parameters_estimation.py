...
#Error in each point of the arc
...
length = float(input("\nEnter initial guess for a in Miyamoto Nagai:\n"))
...
def MN1(x):
    def integ(TheTa, theta):
        R = D_d*TheTa
        MN_Bulge_p= MiyamotoNagaiPotential(amp=Mass,a=length,b=height,normalize=False)
...
def model(parameters, theta1, theta2, sigma):
    ...
    def MN1(x):
        def integ(TheTa, theta):
            R = D_d*TheTa
            MN_Bulge_p= MiyamotoNagaiPotential(amp=MASS,a=A,b=B,normalize=False)
            Sigma = MN_Bulge_p.dens(R,0.)
            kappa = Sigma/SIGMA_CRIT
            return 2*TheTa*np.log(THETA/TheTa)*kappa/(SIGMA_CRIT**2)
        THETA = np.sqrt(x**2 + theta2[l]**2)
...
def lnprior(parameters):
    R_S, m_0, SIGMA_0, H_R, B, A, MASS = parameters
#    if 0.1<R_S<30 and m_0>0 and SIGMA_0>0 and 2<H_R<12 and 0.1<B<15 and 0.1<A<10 and MASS>0:  
    if 0.1<R_S<60.1 and 0.1e11<m_0<1e12 and 1e8<SIGMA_0<1e10 and 2<H_R<30 and 0.1<B<15 and 0.1<A<10 and 0.1e10<MASS<1.5e11:
...
chain_steps = [i for i in range(len(sampler.chain[:,:,6].T))]
chain_W = []
for i in range(nwalkers):
	chain_value = sampler.chain[:,:,6].T[:][:,i]
	ax.plot(chain_steps, chain_value, '-', color='k', alpha=0.3)
ax.plot(chain_steps, len(chain_steps)*[start[6]], '-', color='r', lw=1)
ax.set_xlim(0, len(chain_steps)-1)
plb.savefig('para7.pdf')
...
N=int(input("\nEnter the cut step:\n"))
samples = sampler.chain[:, N:, :].reshape((-1, ndim))
